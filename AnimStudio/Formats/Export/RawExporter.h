...
public:
    ...
    ExportResult exportCurrentFrame(const AnimationData& data, int frameIndex, const QString& outputPath, ImageFormat format, bool updateProgress = false);

    // Export all frames in 'data' to 'outputDir', naming each
    // baseName_frame0.png / .jpg / etc. Returns true only if ALL succeed.
    ExportResult exportAllFrames( const AnimationData& data, const QString& outputDir, ImageFormat format);
